resource "proxmox_vm_qemu" "k8s_worker" {
  ...
  cicustom = "user=local:snippets/k8s-worker-${var.hostname}.yml"
...
resource "local_file" "cloud_init_user_data" {
  ...
  filename = "/var/lib/vz/snippets/k8s-worker-${var.hostname}.yml"
...
output "macaddr" {
  value = proxmox_vm_qemu.k8s_worker.network[0].macaddr
}